...
for i = 1:simulations_number
    ...
    if i == 1
        ...
        hLg = legend({'Reference', 'LMPC', 'FCS-MPC'}, 'Orientation','horizontal','Location','north','Interpreter','latex');
        pos = hLg.Position;
        pos(1) = pos(1) - 0.1;
        pos(2) = pos(2) + 0.07;
        pos(3) = 1.8 * pos(3);
...
end_x_idx = [334, 227, 410];
xlims = [0, 51; -3, 48; 0, 51];
ylims = [-2.25, 4.75; -0.75, 6.25; -3, 4];
...
clearvars -except figures end_x_trajectories xlims ylims simulations_number references AssociateAndGetRMSEGeneral names; clc;
...
for i = 1:simulations_number
    ...
    xlabel('$x\;[m]$','Interpreter','latex');
    ylabel('$y\;[m]$','Interpreter','latex');
    figures{i}.Position = [100 100 2000 275];
    if i == 1
        hLg = legend({'Reference', 'LMPC', 'FCS-MPC'}, 'Orientation','horizontal','Location','north','Interpreter','latex');
        pos = hLg.Position;
        pos(1) = pos(1) - 0.25 * pos(3);
        pos(2) = pos(2) + 0.02;
        pos(3) = 1.5 * pos(3);
        hLg.Position = pos;
        legend('boxoff')
    ...
    set(figures{i},'Units','Inches');
    pos = get(figures{i},'Position');
    set(figures{i},'PaperPositionMode','Auto','PaperUnits','Inches','PaperSize',[pos(3)*0.565, pos(4)*0.93])
    print(figures{i},names{i},'-dpdf','-r0')
    movefile(sprintf('%s.pdf', names{i}), '../../figures/');
end
