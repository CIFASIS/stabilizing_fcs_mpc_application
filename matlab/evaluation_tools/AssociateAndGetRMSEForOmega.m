function [rmse, time_array, distance_array, association_array] = AssociateAndGetRMSEForOmega(logsout, time_table, initial_idx, minimal_distance, end_x)
    % Separate arrays
    idx_x = initial_idx;
    idx_y = idx_x + 1;
    ...
    ref_2 = [time_table(1:2, ref_max_y_idx+1:end); time_table(end, ref_max_y_idx+1:end)];
    ...
    meas_2 = [logsout{idx_x}.Values.Data(meas_max_y_idx+1:end)'; logsout{idx_y}.Values.Data(meas_max_y_idx+1:end)'];
    times_2 = logsout{idx_x}.Values.Time(meas_max_y_idx+1:end);
    if end_x > 0
